...
std::vector<FrameRenderDuration> f_FRDs; // just resize and override
...
void RenderManager::makeRoomForFrames(unsigned int nFrames)
{
	...
	f_FRDs.resize(nFrames);
}

void RenderManager::addLerpFrame(LerpedFrame lf)
{
	m_framesToRender[lf.frameNumber] = lf;
}

void RenderManager::renderFramesSingleThread(std::string outputVideoFile, unsigned int fps, bool deleteFramesAtEnd, std::string outputFolder, std::string frameFileName)
{
...
void RenderManager::renderFramesMultiThread(std::string outputVideoFile, unsigned int fps, bool deleteFramesAtEnd, std::string outputFolder, std::string frameFileName)
{
	//std::sort(m_toRender.begin(), m_toRender.end());
...
void RenderManager::renderFramesMultiThreadPerFrame(std::string outputVideoFile, unsigned int fps, bool deleteFramesAtEnd, std::string outputFolder, std::string frameFileName)
{
	// std::sort(m_toRender.begin(), m_toRender.end());
	unsigned int nFramesToRender = m_numFramesToRender;
	m_statsOutput << "\n___START_RENDERING_INFORMATION___\n";
...
void RenderManager::writeRenderStats(RenderDuration timeToRenderEverything, RenderDuration timeToCovertToVideo)
	{
		...
		*/
		...
		std::sort(f_FRDs.begin(), f_FRDs.end());

		for (int i = 0; i < m_numFramesToRender; i++)
		{
			steady_clock::duration frameRenderTime = f_FRDs[i].end - f_FRDs[i].start;
			milliseconds ms = duration_cast<milliseconds>(frameRenderTime);
			if (ms.count() > 0)
			{
				m_statsOutput << "Frame " << f_FRDs[i].frameNumber << " took " << ms.count() << " milliseconds to render " << std::endl;
			}
		}
...
void RenderManager::renderFrameRange(unsigned int start, unsigned int end, std::string frameFileName, std::string outputFolder)
	{
...
void RenderManager::fiberStartFuncForMultiFrames(uint64_t onInitArgs, uint64_t onRunArgs)
	{
...
void RenderManager::renderFramesStatic(LerpedFrame * lerpedFramesArray, unsigned int start, unsigned int end, std::string ouputFolder, std::string frameFileStart)
	{
		for (unsigned int i = start; i < end; i++)
		{
			FrameRenderDuration frd;
			...
			f_FRDs[i] = frd; // this could be an issue
...
void RenderManager::predictNumSecondsToFinshRendering(unsigned int msToRenderCurrentFrame, unsigned int numFramesLeftToRender)
	{
