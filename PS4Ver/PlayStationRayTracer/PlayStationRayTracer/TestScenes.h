...
namespace TestScenes
{
	...
	void assignKeyFramesToLerpBetween(float timePoint, LerpedFrame * lf, std::vector<KeyFrame> & keyFrames)
	{
		int kf1Index = -1, kf2Index = -1; // treat negative values as unassigned
		...
		lf->a = &keyFrames[kf1Index];
		lf->b = &keyFrames[kf2Index];
	}
	...
	void renderKeyFrameScene(std::string sceneFile)
	{
		...
		*/
		...
		for (int frameNumber = 0; frameNumber < nFramesToRender; frameNumber++)
		{
			...
			LerpedFrame lerpedFrame;

			assignKeyFramesToLerpBetween(frameTimePoint, &lerpedFrame, keyFrames);

			lerpedFrame.frameNumber = frameNumber;
			lerpedFrame.lerpWeight = lerpVal;
			
			rm.addLerpFrame(lerpedFrame);
			/*Frame frameData;
