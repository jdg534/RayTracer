...
#include "Math.h" // my maths library, needed for lerp()

bool Scenes::folderExists(std::string folderStr)
{
	bool exists = true;
#ifdef _WIN32
...
void Scenes::createFolder(std::string folderName)
{
#ifdef _WIN32
...
void Scenes::fillKeyFrameWithMostRecentSphereStates(KeyFrame & kf, std::vector<Sphere> spheres)
{
...
void Scenes::assignKeyFramesToLerpBetween(float timePoint, KeyFrame & kf1, KeyFrame & kf2, std::vector<KeyFrame> & keyFrames)
{
...
float Scenes::weightValueForTimePoints(float min, float max, float point)
{
...
void Scenes::renderKeyFrameScene(std::string sceneFile)
{
